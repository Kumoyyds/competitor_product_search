...
| [cache.py](cache.py) | SQLite cache keyed on extractor version + title for base extraction. Default path `.cache/base_extraction.sqlite`. |
...
| [layers/domain_filter.py](layers/domain_filter.py) | host-vs-`domain_map` match; sets `domain=pass/fail`, kills non-matching candidates. A `domain_map` value ending in `.` is a registrable-name prefix matching any TLD (`amazon.` → `amazon.de` / `amazon.co.uk`); without it, exact host + subdomains |
